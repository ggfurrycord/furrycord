@echo off
:: ─── Furrycord — Publier une nouvelle release sur GitHub ─────────────────────
...
if "%NOTES%"=="" set NOTES=Furrycord %VERSION%

:: Chemins de sortie
set DIST_DIR=dist\desktop
set OUT_DIR=release\installer
set DIST_ZIP=%OUT_DIR%\furrycord-dist.zip
set INSTALLER_EXE=%OUT_DIR%\Furrycord-Installer.exe
set VERSION_JSON=%OUT_DIR%\version.json
set DESKTOP_ASAR=dist\desktop.asar

echo.
echo  ╔═══════════════════════════════════════════════════╗
echo  ║    FURRYCORD — Publication release v%VERSION%
echo  ╚═══════════════════════════════════════════════════╝
echo.
...
:: ── 5. Compiler Furrycord-Installer.exe ──────────────────────────────────────
echo.
echo  [5/8] Compilation de Furrycord-Installer.exe...
...
if not exist "%INSTALLER_EXE%" (
    echo  [ERREUR] Furrycord-Installer.exe introuvable apres compilation.
    pause
    exit /b 1
)

for %%F in ("%INSTALLER_EXE%") do echo  [5/8] Furrycord-Installer.exe cree (%%~zF octets)

:: ── 6. Créer furrycord-dist.zip ──────────────────────────────────────────────
echo.
echo  [6/8] Creation de furrycord-dist.zip...
...
powershell -NoProfile -Command "Add-Type -Assembly System.IO.Compression.FileSystem; $src = (Resolve-Path '%DIST_DIR%').Path; $dst = (Join-Path (Resolve-Path 'release\installer').Path 'furrycord-dist.zip'); [System.IO.Compression.ZipFile]::CreateFromDirectory($src, $dst, [System.IO.Compression.CompressionLevel]::Optimal, $false)"

if not exist "%DIST_ZIP%" (
    echo  [ERREUR] Impossible de creer furrycord-dist.zip
    pause
    exit /b 1
)

for %%F in ("%DIST_ZIP%") do echo  [6/8] furrycord-dist.zip cree (%%~zF octets)
...
(
    ...
    echo   "installerUrl": "https://github.com/furrycordoff/furrycord/releases/latest/download/Furrycord-Installer.exe",
    echo   "distUrl": "https://github.com/furrycordoff/furrycord/releases/latest/download/furrycord-dist.zip",
    echo   "downloadUrl": "https://github.com/furrycordoff/furrycord/releases/latest/download/desktop.asar",
...
gh release create "v%VERSION%" ^
    "%INSTALLER_EXE%#Furrycord-Installer.exe" ^
    "%DIST_ZIP%#furrycord-dist.zip" ^
    "%DESKTOP_ASAR%#desktop.asar" ^
    "%VERSION_JSON%#version.json" ^
    --repo furrycordoff/furrycord ^
    --title "Furrycord v%VERSION%" ^
    --notes "!NOTES!" ^
    --latest
...
echo.
echo  ╔═══════════════════════════════════════════════════════════════╗
echo  ║  Furrycord v%VERSION% publie avec succes !
echo  ║
echo  ║  Fichiers publies sur GitHub :
echo  ║    Furrycord-Installer.exe    — installeur .exe avec GUI
echo  ║    furrycord-dist.zip         — JS obfusques (pour l'injec.)
echo  ║    version.json               — metadonnees de version
echo  ║
echo  ║  Les utilisateurs telechargeront Furrycord-Installer.exe
