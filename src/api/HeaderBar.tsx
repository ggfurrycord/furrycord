    ...
    return (
        <Tooltip text={tooltip ?? ""} position={position} shouldShow={tooltip != null}>
            {({ onMouseEnter, onMouseLeave }) => (
                <Clickable
                    {...{ innerRef: ref } as any}
                    className={classes(HeaderBarClasses?.actionButton, HeaderBarClasses?.highlight, "vc-header-bar-btn", className)}
                    style={{ width: iconSize, boxSizing: "content-box", justifyContent: "center", color: "oklab(0.745437 0.00131872 -0.00849736)", margin: "0 4px", cursor: "pointer", background: "transparent", border: "none", boxShadow: "none" }}
                    onClick={onClick}
...
try { _stealthActive = localStorage.getItem("Furrycord_stealthMode") === "1"; } catch { }
...
function persistStealth(v: boolean) {
    try { v ? localStorage.setItem("Furrycord_stealthMode", "1") : localStorage.removeItem("Furrycord_stealthMode"); } catch { }
}

// Éléments NON-React uniquement (titlebar, FurrycordAI nav)
// NE PAS cacher les entrées settings sidebar — l'utilisateur doit pouvoir accéder aux paramètres
const NON_REACT_SELECTORS = [
    "#furrycord-titlebar-btn",
    "#furrycord-titlebar-link-style",
    ".nai-nav-item",
];
...
export function syncStealthBodyClass() {
    try { if (_stealthActive) document.body?.classList.add("furrycord-stealth"); else document.body?.classList.remove("furrycord-stealth"); } catch { }
...
if (_stealthActive) {
    ...
    try { document.body?.classList.add("furrycord-stealth"); } catch { }
}
...
try {
    ...
    window.addEventListener("furrycord-stealth-change", () => {
...
export function _notifyStealthChange() {
    ...
    window.dispatchEvent(new Event("furrycord-stealth-change"));
}
...
function HeaderBarButtons() {
    ...
    useEffect(() => {
        ...
        window.addEventListener("furrycord-stealth-change", listener);
        return () => {
            headerBarListeners.delete(listener);
            stealthListeners.delete(listener);
            window.removeEventListener("furrycord-stealth-change", listener);
        };
    }, []);
