...
import { t } from "../autoTranslateFurrycord";
...
const MI_TOKEN_CACHE_KEY = "furrycord-mi-token-cache";
...
function ContextMenuPortal(props: CtxMenuProps) {
    ...
    const [container] = React.useState(() => {
        const el = document.getElementById("furrycord-mi-ctx-root") ?? document.createElement("div");
        el.id = "furrycord-mi-ctx-root";
...
export default definePlugin({
    ...
    async start() {
        ...
        captureCurrentToken();
        addHeaderBarButton("furrycord-multi-instance", () => <MultiInstanceButton />, 9);
    },

    stop() {
        removeHeaderBarButton("furrycord-multi-instance");
        if (this._fluxUnsub) { this._fluxUnsub(); this._fluxUnsub = null; }
        const root = document.getElementById("furrycord-mi-ctx-root");
        if (root) root.remove();
    },
});
