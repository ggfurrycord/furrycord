...
import { registerMediaPermissionsForSession } from "../../furrycord/main/mediaPermissions";
...
function createTokenPreload(token: string): string {
    ...
    const dir = join(app.getPath("userData"), "furrycord-mi-preloads");
    mkdirSync(dir, { recursive: true });

    const safeToken = JSON.stringify(token); // échappe proprement le token

    const script = `
// Furrycord MultiInstance — token preload
// S'exécute dans le main world AVANT Discord
(function() {
    const TOKEN = ${safeToken};
    try {
        // Définit le token dans localStorage
        Object.defineProperty(window, '__furrycord_token', { value: TOKEN, writable: false });
        ...
        console.log("[FurrycordMI] Token preload active ✓");
    } catch(e) {
        console.warn("[FurrycordMI] Preload error:", e);
    }
})();
`;
...
export async function openInstanceWindow(
    ...
    try {
        ...
        const uniqueAppId = `furrycord.instance.${userId}.${Date.now()}`;
        ...
        const partition = `persist:furrycord-mi-${userId}`;
        ...
        const win = new BrowserWindow({
            ...
            title: `Furrycord [${username || userId}]`,
        ...
        if (process.platform === "win32") {
            try {
                win.setAppDetails({
                    ...
                    relaunchDisplayName: `Furrycord [${username || userId}]`,
                });
            } catch (err) {
                console.warn("[FurrycordMI] setAppDetails failed:", err);
            }
        }
            ...
            wc.executeJavaScript(`
                (async () => {
                    ...
                    try {
                        ...
                        const ws = window.__FURRYCORD_GW_WS__;
...
// ─────────────────────────────────────────────────────────────────────────────
// Fenetres « groupées » — meme groupe que Furrycord dans la barre des taches
// Principe : on ne touche PAS a setAppDetails => la fenetre herite de l'AppId
// du processus principal (com.furrycord.app), Windows la groupe automatiquement
// ─────────────────────────────────────────────────────────────────────────────
...
export async function arrangeSplit(_: any, userId: string): Promise<void> {
    try {
        ...
        console.error("[FurrycordMI] arrangeSplit error:", e);
    }
}
