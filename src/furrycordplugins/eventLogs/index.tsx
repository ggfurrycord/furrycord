...
import { t, useTranslation } from "../autoTranslateFurrycord";
...
function loadPersistLogs() {
    try {
        const s = localStorage.getItem("furrycord_logs");
        if (s) {
...
function savePersistLogs() {
    try {
        ...
        localStorage.setItem("furrycord_logs", JSON.stringify(toSave));
    } catch { }
}
...
function LogRow({ e }: { e: LogEntry; }) {
    ...
    const onDoubleClick = () => {
        ...
        try {
            ...
            console.error("Furrycord Navigation Error:", err);
...
function LogsModal({ rootProps }: { rootProps: any; }) {
    ...
    const saveAsTxt = useCallback(() => {
        try {
            ...
            a.href = url;
            a.download = `furrycord_logs_${new Date().toISOString().slice(0, 10)}.txt`;
            a.click();
...
export default definePlugin({
    ...
    start() {
        ...
        loadPersistLogs();
        addHeaderBarButton("furrycord-event-logs", () => <LogsButton />, 7);
        subscribeToEvents();
    },
    stop() {
        removeHeaderBarButton("furrycord-event-logs");
