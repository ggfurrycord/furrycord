...
import { EQUIBOT_USER_ID, FURRYCORD_BOT_USER_ID } from "@utils/constants";
...
export function ChatPluginCard({ url, description }: { url: string, description: string; }) {
    ...
    if (excludedPlugin || !p) {
        const toolTipText = excludedPlugin
            ...
            : "This plugin is not on this version of Furrycord. Try updating!";
...
export const PluginCards = ErrorBoundary.wrap(function PluginCards({ message }: { message: Message; }) {
    ...
    // Process components — Furrycord Bot (furrycord.su, Component v2 Container format)
    if (message.author.id === FURRYCORD_BOT_USER_ID) {
        ...
        if (containerComponents?.length >= 3) {
            ...
            if (pluginUrl?.startsWith("https://furrycord.su/plugins/")) {
