...
export const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "equibop" | "desktop" | "dev", string> = {
    ...
    dev: "Developer version of Furrycord"
};
...
export default function PluginSettings({ premiumOnly = false }: PluginSettingsProps) {
    ...
    const [searchValue, setSearchValue] = useState({ value: "", status: SearchStatus.FURRYCORD });
    ...
    React.useEffect(() => {
        ...
        window.addEventListener("furrycord-tutorial-detected", handler);
        return () => window.removeEventListener("furrycord-tutorial-detected", handler);
    }, []);
    ...
    const pluginFilter = useCallback((plugin: typeof Plugins[keyof typeof Plugins], newPluginsSet: Set<string> | null) => {
        ...
        switch (status) {
            ...
            case SearchStatus.ENABLED:
                if (!enabled) return false;
                break;
            case SearchStatus.FURRYCORD:
                if (!PluginMeta[plugin.name].folderName.startsWith("src/furrycordplugins/")) return false;
                break;
            case SearchStatus.OTHERS:
                if (PluginMeta[plugin.name].folderName.startsWith("src/furrycordplugins/") || PluginMeta[plugin.name].folderName.startsWith("src/plugins/_")) return false;
    ...
    const { furrycordPlugins, othersPlugins, requiredPlugins } = useMemo(() => {
        const furrycordPlugins = [] as JSX.Element[];
        ...
        for (const p of sortedPlugins) {
            ...
            if (isRequired) {
                const tooltipText = p.required || !depMap[p.name]
                    ? "This plugin is required for Furrycord to function."
                ...
                const isFurrycord = folderName.startsWith("src/furrycordplugins/");
                const card = (
                    <PluginCard
                        onRestartNeeded={handleRestartNeeded}
                        disabled={false}
                ...
                if (isFurrycord) {
                    furrycordPlugins.push(card);
                } else {
                    othersPlugins.push(card);
                }
            }
        }
        return { furrycordPlugins, othersPlugins, requiredPlugins };
    ...
    const allPlugins = [...furrycordPlugins, ...othersPlugins];
    ...
    const furrycordVisible = furrycordPlugins.slice(0, Math.min(visibleCount, furrycordPlugins.length));
    const othersVisible = othersPlugins.slice(0, Math.max(0, visibleCount - furrycordPlugins.length));
        ...
        <SettingsTab>
                    ...
                    <ErrorBoundary noop>
                        <Select
                            options={[
                                ...
                                { label: "Show Furrycord Plugins", value: SearchStatus.FURRYCORD },
            ...
            {premiumOnly ? (
                <>
                    ...
                    {furrycordPlugins.length || othersPlugins.length
                        ? (
                            <div className={cl("grid")}>
                                {[...furrycordVisible, ...othersVisible].length
                                    ? [...furrycordVisible, ...othersVisible]
                ...
                <>
                    {furrycordPlugins.length > 0 && (
                        <>
                            <HeadingTertiary className={Margins.top20}>Furrycord Plugins</HeadingTertiary>
                            <div className={cl("grid")}>
                                {furrycordVisible}
                            </div>
                        </>
                    ...
                    {furrycordPlugins.length === 0 && othersPlugins.length === 0 && (
                        <ExcludedPluginsList search={search} />
                    )}
