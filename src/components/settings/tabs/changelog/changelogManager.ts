...
const FURRYCORD_RELEASES_REPO = "furrycordoff/furrycord";
const FURRYCORD_REPO_URL = `https://github.com/${FURRYCORD_RELEASES_REPO}`;
...
export { FURRYCORD_REPO_URL };

export async function getCommitsSinceLastSeen(
    repoUrl: string,
): Promise<ChangelogEntry[]> {
    // Toujours utiliser le repo Furrycord, ignorer le repoUrl d'Equicord
    return fetchCommitsBetween(FURRYCORD_RELEASES_REPO, "HEAD~10", "HEAD").catch(() => []);
}
