...
function createFurrycordMenuItems(): MenuItemConstructorOptions[] {
    return [
        {
            label: "Furrycord",
            submenu: [
                {
                    label: "About Furrycord",
                    click: () => openAboutWindow()
                },
                {
                    label: cachedUpdateAvailable ? "Update Furrycord" : "Check for Updates",
...
export function patchTrayMenu(): void {
    // On ne patch plus le menu tray — Furrycord gère son propre tray dans patcher.ts
}
