...
const RELEASES_REPO = "furrycordoff/furrycord";
...
const ZIP_FILE = "furrycord-dist.zip";
...
async function applyUpdates(): Promise<boolean> {
    ...
    try {
        ...
        // Save zip to temp
        const zipPath = join(app.getPath("temp"), `furrycord-update-${Date.now()}.zip`);
        ...
        const tmpExtract = join(app.getPath("temp"), `furrycord-extract-${Date.now()}`);
...
app.on("before-quit", event => {
    ...
    console.log("[Furrycord] Applying pending update before quit...");

    const safetyTimeout = setTimeout(() => {
        console.error("[Furrycord] Update on quit timed out — forcing exit.");
    ...
    applyUpdates()
        .then(ok => {
            if (ok) console.log("[Furrycord] Update applied successfully on quit.");
            else console.warn("[Furrycord] Update on quit returned false.");
        })
        .catch(err => {
            console.error("[Furrycord] Update on quit failed:", err);
