...
async function runMacOSAppleScriptSequence(
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "furrycord-wb-macos-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_TYPE, async (event, text: string, delay: number = 50) => {
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "furrycord-wb-"));
...
function runPowershellScript(psScript: string): Promise<void> {
    ...
    const { join } = require("path");
    const { tmpdir } = require("os");
    const tempDir = mkdtempSync(join(tmpdir(), "furrycord-ps-"));
...
ipcMain.handle(IpcEvents.WORLD_BOMB_SEQUENCE, async (
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "furrycord-wbs-"));
...
`;
    ...
    const tempDir = mkdtempSync(join(tmpdir(), "furrycord-kb-"));
...
ipcMain.handle(IpcEvents.OPEN_MONACO_EDITOR, async () => {
    ...
    monacoWin = new BrowserWindow({
        title: "Furrycord QuickCSS Editor",
...
const OFFICIAL_UPDATE_URL = "https://github.com/furrycordoff/furrycord/releases/latest/download/Furrycord-Installer.exe";

ipcMain.handle(IpcEvents.FURRYCORD_DOWNLOAD_AND_RUN, async (_, url: string) => {
    ...
    const tmpPath = path.join(os.tmpdir(), "FurrycordUpdate-Setup.exe");
    ...
    const { response } = await dialog.showMessageBox({
        ...
        defaultId: 0,
        title: "Mise à jour Furrycord",
        message: "Une mise à jour de Furrycord est disponible.",
...
ipcMain.handle(IpcEvents.INSTALL_VB_CABLE, async () => {
    ...
    const tmpDir = path.join(os.tmpdir(), "Furrycord-VBCable");
    ...
    try {
        ...
        console.error("[Furrycord] VBCable install failed:", err);
