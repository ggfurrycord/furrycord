...
if (!IS_VANILLA) {
    ...
    class BrowserWindow extends electron.BrowserWindow {
        constructor(options: BrowserWindowConstructorOptions) {
            if (options?.webPreferences?.preload && options.title) {
                ...
                // Custom Furrycord branding icon (set by installer)
                if (isMainWindow && process.platform === "win32") {
                    const brandIcon = join(process.env.LOCALAPPDATA ?? "", "Furrycord", "furrycord.ico");
                    if (fsExistsSync(brandIcon)) {
                        options.icon = brandIcon;
                    }
                }
                ...
                if (process.platform === "win32" && winMaterial && winMaterial !== "none") {
                    try {
                        ...
                        if (!applied) {
                            console.warn("[Furrycord] No background material API available on this system");
                        }
                    } catch (e) {
                        console.error("[Furrycord] setBackgroundMaterial failed:", e);
                    }
                }
    ...
    process.env.DATA_DIR = join(app.getPath("userData"), "..", "Furrycord");
        ...
        (electron.ipcMain as any).handle = function(channel: string, listener: any) {
            ...
            try {
                ...
                if (e?.message?.includes?.("Attempted to register a second handler")) {
                    console.warn(`[Furrycord] Ignored duplicate IPC handler for '${channel}'`);
                    return;
                }
                throw e;
            }
    ...
    console.log("[Furrycord] Running in vanilla mode. Not loading Furrycord");
}

console.log("[Furrycord] Loading original Discord app.asar");
