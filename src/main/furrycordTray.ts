...
/**
 * Initializes the Furrycord system tray icon.
 ...
 */
export function initFurrycordTray(injectorPath: string) {
    ...
    app.whenReady().then(() => {
        try {
            ...
            tray.setToolTip("Furrycord");

            const trayMenu = Menu.buildFromTemplate([
                {
                    label: "Open Furrycord",
                    click() {
                ...
                { type: "separator" },
                {
                    label: "Uninstall Furrycord",
                    click() {
                        const uninstallerCandidates = [
                            join(process.env.LOCALAPPDATA || "", "Programs", "furrycord", "Uninstall Furrycord.exe"),
                            join(process.env.LOCALAPPDATA || "", "Programs", "Furrycord", "Uninstall Furrycord.exe"),
                            join(process.env.PROGRAMFILES || "", "Furrycord", "Uninstall Furrycord.exe"),
                        ];
            ...
            console.log("[Furrycord] Tray created ✓", iconPath ?? "(icon from exe)");
        } catch (e) {
            console.error("[Furrycord] Failed to create tray:", e);
        }
    });
}
