...
export const DATA_DIR = process.env.FURRYCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "FurrycordData", suffix)
        : join(app.getPath("userData"), "..", "Furrycord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[Furrycord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
