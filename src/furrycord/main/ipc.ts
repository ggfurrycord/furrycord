...
handleSync(IpcEvents.GET_GIT_HASH, () => Furrycord_GIT_HASH);
...
// Handler pour VencordNative.furrycord.relaunch() — utilisé par les boutons Restart dans les settings plugins
...
handleSync(IpcEvents.IS_USING_CUSTOM_VENCORD_DIR, () => !!State.store.FurrycordDir);
handle(IpcEvents.SHOW_CUSTOM_VENCORD_DIR, async () => {
    const { FurrycordDir } = State.store;
    if (!FurrycordDir) return;

    const stats = await stat(FurrycordDir);
    if (!stats.isDirectory()) return;

    shell.openPath(FurrycordDir);
});
...
handle(IpcEvents.SELECT_VENCORD_DIR, async (_e, value?: null) => {
    if (value === null) {
        delete State.store.FurrycordDir;
        return "ok";
    }
