...
const options = {
    ...
    repair: {
        type: "boolean",
        short: "r",
        description: "Re-download Furrycord and restart"
    }
...
const extraOptions = {
    ...
    "ozone-platform": {
        hidden: process.platform !== "linux",
        type: "string",
        description: "Whether to run Furrycord in Wayland or X11 (XWayland)",
        options: ["x11", "wayland"]
    }
} satisfies Record<string, Option>;
...
export async function checkCommandLineForRepair() {
    ...
    if (State.store.FurrycordDir) {
        console.error("Cannot repair: using custom Furrycord directory. Remove it in settings first.");
        process.exit(1);
    }

    console.log("Repairing Furrycord...");
...
export function checkCommandLineForHelpOrVersion() {
    ...
    if (version) {
        console.log(`Furrycord v${app.getVersion()}`);
        app.exit(0);
    }

    if (help) {
        const base = stripIndent`
            Furrycord v${app.getVersion()}
...
function checkCommandLineForToggleCommands() {
    ...
    console.error("Furrycord is not running. Toggle commands require a running instance.");
    app.exit(1);
}
...
function checkForSecondInstance() {
    ...
    if (!app.requestSingleInstanceLock({ IS_DEV })) {
        if (IS_DEV) {
            console.log("Furrycord is already running. Quitting previous instance...");
            return;
        } else {
            console.log("Furrycord is already running. Quitting...");
            app.exit(0);
        }
    }
