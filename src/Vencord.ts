...
import "~plugins";
console.log("%c[Furrycord]", "color: #5865f2; font-weight: bold;", "Injection successful! Starting services...");
...
function showGreenUpdateBanner() {
    if (document.getElementById("furrycord-core-updater-root")) return;

    const banner = document.createElement("div");
    banner.id = "furrycord-core-updater-root";
    ...
    titleSpan.textContent = "🔔 Furrycord Update Available!";
...
function initTrayIpc() {
    ...
    VencordNative.tray.onCheckUpdates(async () => {
        try {
            ...
            if (isOutdated) {
                showNotice("A Furrycord update is available!", "View Update", () => openSettingsTabModal(UpdaterTab!));
            } else {
    ...
    VencordNative.tray.onRepair(async () => {
        try {
            ...
            UpdateLogger.error("Failed to repair Furrycord", err);
        }
    });
    ...
    if (IS_DEV) {
        ...
        if (pendingPatches.length)
            PMLogger.warn(
                ...
                "You are seeing this warning because this is a Development build of Furrycord.",
