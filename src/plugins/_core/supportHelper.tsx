...
import { platformName } from "@furrycordplugins/equicordHelper/utils";
...
export default definePlugin({
    ...
    renderContributorDmWarningCard: ErrorBoundary.wrap(({ channel }) => {
        ...
        // Groupe 2 : Message "Furrycord developers"
        const furrycordGroupIds = [
        ...
        const isFurrycordGroup = furrycordGroupIds.includes(userId);

        if (!isOriginalDev && !isFurrycordGroup) return null;
        if (RelationshipStore.isFriend(userId) || isAnyPluginDev(UserStore.getCurrentUser()?.id)) return null;

        const developerText = isOriginalDev ? "Equicord & Vencord plugin developers" : "Furrycord developers";
