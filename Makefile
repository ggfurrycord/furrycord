...
RC_OBJ = $(BUILD_DIR)/furrycord_rc.o
...
all: $(BUILD_DIR)/furrycord $(BUILD_DIR)/updater.exe
...
$(BUILD_DIR)/furrycord_rc.o: furrycord.rc assets/furrycord.ico
	mkdir -p $(BUILD_DIR)
	windres furrycord.rc -o $@

$(BUILD_DIR)/%.o: %.c
	mkdir -p $(dir $@)
	$(CC) -c -o $@ $(FLAGS) $< -Wall -Wno-unused-function -Wno-misleading-indentation $(OPTS)

$(BUILD_DIR)/furrycord: $(OBJS) $(BUILD_DIR)/resources.o $(RC_OBJ) $(BUILD_DIR)/themes/ $(BUILD_DIR)/sounds/
...
uninstall:
	rm -f $(PREFIX)/share/applications/furrycord.desktop
	rm -f $(PREFIX)/share/pixmaps/furrycord.png
	rm -f $(PREFIX)/share/pixmaps/furrycord.svg
	rm -f $(PREFIX)/bin/furrycord

install: uninstall
	cp furrycord.desktop $(PREFIX)/share/applications/furrycord.desktop
	cp assets/icon.svg $(PREFIX)/share/pixmaps/furrycord.svg
	cp $(BUILD_DIR)/furrycord $(PREFIX)/bin/furrycord

run: all
	cp assets/furrycord.gschema.xml $(BUILD_DIR)/assets/furrycord.gschema.xml
