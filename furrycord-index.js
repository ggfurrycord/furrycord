...
// ── CRITIQUE : userData = dossier Furrycord pour les settings/plugins
...
const furrycordData = path.join(app.getPath("appData"), "Furrycord");
app.setPath("userData", furrycordData);
...
// pour le token Discord stable. Quand Furrycord charge ce cache, il lit des permissions
...
// AppUserModelId unique — Windows reconnaît Furrycord comme app séparée de Discord
...
app.once("ready", () => {
    try {
        ...
        //     Seuls les modules vraiment inutiles pour Furrycord sont bloqués.
        ...
        app.once("browser-window-created", () => {
            try {
                ...
                ses.webRequest.onBeforeRequest(
                    ...
                    (details, callback) => {
                        ...
                        if (isBlocked) {
                            ...
                            console.log("[Furrycord] Module bloqué (inutile pour Furrycord):", url.split("/").slice(-2).join("/"));
                ...
                console.log("[Furrycord] Filtre modules 403 activé ✓");
            } catch (e) {
                console.warn("[Furrycord] Impossible d'activer le filtre modules:", e.message);
            }
        });
    } catch (e) {
        console.warn("[Furrycord] FIX modules 403 failed:", e.message);
    }
});
...
try {
    const lsPath = path.join(furrycordData, "Local Storage", "leveldb");
    if (fs.existsSync(lsPath)) {
        ...
        if (corrupted) {
            console.warn("[Furrycord] LevelDB localStorage corrompu détecté — réparation...");
            try { fs.rmSync(lsPath, { recursive: true, force: true }); } catch { }
            console.warn("[Furrycord] LevelDB supprimé — les données localStorage seront récréées");
        }
    }
} catch (e) { console.warn("[Furrycord] LevelDB check failed:", e.message); }

// Modules bundlés dans furrycord-dist/modules/
...
try {
    ...
    if (entries.length > 0) {
        ...
        console.log("[Furrycord] Modules natifs Discord détectés:", discordNativeModulesPath);
    }
} catch (e) {
    console.warn("[Furrycord] Impossible de détecter les modules natifs Discord:", e.message);
}
...
// Priorité aux modules bundlés (portables, dans furrycord-dist/modules/)
...
if (discordNativeModulesPath) {
    ...
    } catch (e) { console.warn("[Furrycord] Erreur lors du scan des modules natifs:", e.message); }
}
try {
...
console.log("[Furrycord] mainAppDirname:", global.mainAppDirname);
...
try {
    ...
    if (fs.existsSync(nativeModulesDir) && !buildInfo.localModulesRoot) {
        ...
        console.log("[Furrycord] build_info.json patché → localModulesRoot:", nativeModulesDir);
    }
} catch (e) {
    console.warn("[Furrycord] Impossible de patcher build_info.json:", e.message);
}
